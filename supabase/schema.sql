...
CREATE TABLE IF NOT EXISTS programs (
  ...
  progression_rules JSONB, -- Phase 30D: progression rules array [{id?,label,text}]
  created_at TIMESTAMPTZ DEFAULT NOW(),
  updated_at TIMESTAMPTZ DEFAULT NOW()
);
...
-- ============================================================
-- Phase 27E: meal plans (one active plan per client in v1;
-- items as jsonb). RLS mirrors client_goals.
-- ============================================================
...
-- ============================================================
-- Phase 53: trial intake (clients.equipment_access) + trial
-- assessments (trainer records what a new client can/can't do).
...
-- ============================================================
-- Phase 67: daily_plan_items (mirror of supabase/daily-plan-items.sql,
-- applied live via pooler 2026-09-02). Custom plan items persist;
-- session/target/check-in auto items are derived at render, never stored.
-- ============================================================
...
-- ============================================================
-- Phase 76: demo dashboard stats RPC (public /demo page)
-- ============================================================
...
-- ============================================================
-- Phase 95: Notification triggers — idempotency log + due-alerts
-- evaluator. Applied live via pooler (supabase/notification-triggers-95.sql).
-- The PUSH SEND path is the notification-worker edge function
-- (supabase/functions/notification-worker) — deploy is an owner action
-- (no SUPABASE_ACCESS_TOKEN on the build machine).
-- ============================================================
...
-- Due-alerts evaluator: one row per due alert across ALL account-having
-- clients, suppression PRE-APPLIED in suppressed_reason:
...
-- deferred. Timezone: day boundaries and quiet-hours are evaluated in
-- the DATABASE timezone (UTC); the app has no per-user timezone, so
-- near-midnight local boundaries can shift by the client's UTC offset
...
-- ============================================================
-- Phase 96b: expenses + net profit (money dashboard)
-- ============================================================
